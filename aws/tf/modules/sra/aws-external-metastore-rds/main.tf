...
locals {
  lib_dir = "modules/sra/aws-external-metastore-rds/lib"
}
...
resource "databricks_cluster" "this" {
  ...
  spark_env_vars = {
    ...
    "DBFS_LIB": "${var.dbfs_lib_location}",
...
resource "null_resource" "exec_metastore_init" {
  ...
  provisioner "local-exec" {
    command = "curl -X POST -H 'Authorization: Bearer ${databricks_token.this.token_value}' '${data.databricks_current_user.me.workspace_url}/api/2.1/jobs/run-now' -d '${jsonencode({ "job_id": "${databricks_job.this.id}"})}'"
    interpreter = ["bash", "-c" ]
  }
}