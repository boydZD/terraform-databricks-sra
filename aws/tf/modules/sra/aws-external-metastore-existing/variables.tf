# variable "databricks_workspace_url" {
#   #default = ""
#   description = "URI: https://<workspace-name>.staging.cloud.databricks.com"
# }

# variable "vpc_id" {
#   #default = ""
#   description = "VPC ID the workspace is deployed in; used to associate subnets with the new RDS instance"
# }

# variable "workspace_security_group_id" {
#   #default = ""
#   description = "Security Group ID for the classic compute plane. The RDS instance will be placed within this group."
# }

variable "dbfs_lib_location" {
  default = "tmp/databricks/metastore-init"
  description = "DBFS location to collect uploaded libraries"
}

# variable "hive_user" {
#   default = "metastore"
#   description = "HMS connection user"
# }

variable "hive_database" {
  default = "organization0"
  description = "Name of HMS database"
}

# variable "hive_pword" {
#   description = "HMS password"
#   sensitive = true
# }

variable "rds_pword" {
  description = "HMS password"
  sensitive = true
}

variable "rds_host" {
  description = "RDS host for the HMS"
}

variable "rds_user" {
  description = "HMS connection user"
}

variable "rds_port" {
  description = "RDS host port for the HMS"
  default = "3306"
}


variable "hive_version" {
  type = string
  default = "2.3.9"
  description = "Version of Hive to use in the metastore; 2.3.9"
}

variable "hive_lib_version" {
  type = string
  default = "3.1.3"
  description = "Version of Hive libraries to use for metastore setup only; should always be 3.1.0+"
}

variable "hadoop_version" {
  type = string
  default = "2.9.2"
  description = "Version of Hive to use"
}

variable "tags" {
  default = {
    Owner = "someone@databricks.com"
    Project = "dev"
  }
}

# variable "region" {
#   default = "us-east-1"
# }

variable "project_name" {
  default = "dev-privatelink"
  description = "workspace name"
}

# resource "random_string" "naming" {
#   special = false
#   upper   = false
#   length  = 6
# }

# locals {
#   prefix = "ext-hms-${random_string.naming.result}"
# }
